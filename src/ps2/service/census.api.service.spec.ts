...
const retries = 10; // Number of retries for Census requests
...
describe("CensusApiService", () => {
  ...
  describe("onModuleInit", () => {
    it(
      "should crash upon missing service ID",
      async () => {
        TestBootstrapper.setupConfig(moduleRef, {
          ps2: {
            censusServiceId: "",
        ...
        await expect(service.onModuleInit()).rejects.toThrow(
          "PS2_CENSUS_SERVICE_ID is not defined.",
        );
      },
      testTimeout,
    );
    ...
    it(
      "should throw upon census being unavailable or errors",
      async () => {
  ...
  describe("sendRequest", () => {
    it(
      "should return a successful request",
      async () => {
    ...
    it(
      "should throw when no response data was returned",
      async () => {
        mockClient.get = jest.fn().mockResolvedValue({});

        await expect(service.sendRequest(mockUrl)).rejects.toThrow(
          "No data was received from Census!",
        );
    ...
    it(
      "should retry upon a Census service error message",
      async () => {
        const mockResponse = { data: "mock data" };
        mockClient.get = jest
          .fn()
          .mockResolvedValueOnce({ data: { error: "Service Unavailable" } })
    ...
    it(
      'should retry upon a Census "errorMessage"',
      async () => {
        const mockResponse = { data: "mock data" };
        mockClient.get = jest
          .fn()
          .mockResolvedValueOnce({
            data: { errorMessage: "Some Census Error" },
          })
          .mockResolvedValue(mockResponse);
    ...
    it(
      "should retry upon a Census error, and throw an error when retries are exceeded",
      async () => {
        const lastError = "Service Unavailable";
        mockClient.get = jest
          .fn()
          .mockResolvedValue({ data: { error: lastError } });

        await expect(service.sendRequest(mockUrl)).rejects.toThrow(
          `Failed to perform request to Census after ${retries} retries. Final error: "Census responded with an error: "${lastError}""`,
        );

        expect(mockClient.get).toHaveBeenCalledWith(mockUrl);
        expect(mockClient.get).toHaveBeenCalledTimes(retries);
      },
      testTimeout,
    );

    it(
      "should retry upon a Census errorMessage, and throw an error when retries are exceeded",
      async () => {
        const lastError = "Some error from Census";
        mockClient.get = jest
          .fn()
          .mockResolvedValue({ data: { errorMessage: lastError } });
  ...
  describe("getCharacter", () => {
    it(
      "should return a character, and also returns outfit details",
      async () => {
        ...
        service.sendRequest = jest
          .fn()
          .mockResolvedValue({ character_list: [mockCharacter] });
    ...
    it(
      "should throw an error if a character doesn't exist",
      async () => {
        const name = "IDoNotExist101010101010101";
        service.sendRequest = jest
          .fn()
          .mockResolvedValue({ character_list: [] });

        await expect(service.getCharacter(name)).rejects.toThrow(
          `Character \`${name}\` does not exist. Please ensure you have spelt it correctly.`,
        );
      },
      testTimeout,
    );
    ...
    it(
      "should throw an error if a character search exhausts its retries",
      async () => {
        const name = "IDoNotExist101010101010101";
  ...
  describe("getCharacterById", () => {
    it(
      "should return a character with the same ID",
      async () => {
    ...
    it(
      "should throw an error if a character doesn't exist (by ID)",
      async () => {
        ...
        await expect(service.getCharacterById(id)).rejects.toThrow(
    ...
    it(
      "should throw an error when Census falls on its arse",
      async () => {
        service.sendRequest = jest.fn().mockImplementation(() => {
          throw new CensusServerError("Census errored.");
        });

        await expect(service.getCharacterById("123456")).rejects.toThrow(
          "Census Errored when fetching character with ID **123456**. Err: Census errored.",
        );
      },
      testTimeout,
    );
  ...
  describe("getOutfit", () => {
    it(
      "should return an outfit",
      async () => {
        const mockOutfitId = "454544545769833";
        const mockOutfit = TestBootstrapper.getMockPS2Outfit(mockOutfitId);
        service.sendRequest = jest
          .fn()
          .mockResolvedValue({ outfit_list: [mockOutfit] });
    ...
    it(
      "should throw an error when the outfit does not exist",
      async () => {
        const outfitId = "123434354654";
        service.sendRequest = jest.fn().mockResolvedValue({ outfit_list: [] });

        await expect(service.getOutfit(outfitId)).rejects.toThrow(
    ...
    it(
      ...
      async () => {
        ...
        await expect(service.getOutfit("123456")).rejects.toThrow(
          "Census Errored when fetching outfit with ID **123456**. Err: Census errored.",
        );
      },
      testTimeout,
    );
