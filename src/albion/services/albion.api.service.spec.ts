...
describe("AlbionApiService", () => {
  ...
  it("should return an error if no player exists", async () => {
    ...
    jest
      .spyOn(AlbionAxiosFactory.prototype, "createAlbionApiAmericasClient")
      .mockReturnValue({
        defaults: {
  ...
  it("should return a character based on exact match amongst partial matches", async () => {
    ...
    await expect(
      service.getCharacter("Maelstrome", AlbionServer.AMERICAS),
    ).resolves.toStrictEqual(properResult);
  });
  ...
  it("should return a character based on an ID", async () => {
    ...
    await expect(
      service.getCharacterById(id, AlbionServer.AMERICAS),
    ).resolves.toStrictEqual(properResult);
  });
  ...
  it("should handle a character having duplicates, as long as only one of them is in the guild", async () => {
    ...
    await expect(
      service.getCharacter(characterName, AlbionServer.AMERICAS),
    ).resolves.toStrictEqual(properResult);
  });
  ...
  it("should throw error when multiple characters of the same name in the guild", async () => {
    ...
    await expect(
      ...
      `multiple characters for **NightRaven2511** were found within the DIG guild. This is an unsupported use case for this registration system. Pinging <@${TestBootstrapper.mockConfig.discord.devUserId}>!`,
    );
  });
  ...
  it("should get all guild members", async () => {
    ...
    const receivedMembers = await service.getAllGuildMembers(
      guildId,
  ...
  describe("Europe", () => {
    it("should properly return a character using expected client generated by createAlbionApiEuropeClient", async () => {
      ...
      await expect(
        service.getCharacterById(id, AlbionServer.EUROPE),
      ).resolves.toStrictEqual(properResult);
    });
    it("should return a character based on exact match amongst partial matches", async () => {
      ...
      await expect(
        service.getCharacter("Lilith1", AlbionServer.EUROPE),
      ).resolves.toStrictEqual(correctResult);
    });
    it("should return a character named Dedalus17", async () => {
      ...
      await expect(
        service.getCharacter("Dedalus17", AlbionServer.EUROPE),
      ).resolves.toStrictEqual(correctResult);
    });
    it("should return a character ID for Lugasi", async () => {
      const correctResultJson =
        '{"guilds":[],"players":[{"Id":"JRlDBmqHS86m764x-jc96g","Name":"Lugasi","GuildId":"0_zTfLfASD2Wtw6Tc-yckA","GuildName":"Dignity Of War","AllianceId":"t4Fl03pZRzaYTEczrDAesA","AllianceName":"DIG","Avatar":"","AvatarRing":"","KillFame":1391737,"DeathFame":2094479,"FameRatio":0.66,"totalKills":null,"gvgKills":null,"gvgWon":null}]}';
      const searchResponse = {
      ...
      await expect(
        service.getCharacterId("Lugasi", AlbionServer.EUROPE),
      ).resolves.toStrictEqual("JRlDBmqHS86m764x-jc96g");
    });
  });
});
