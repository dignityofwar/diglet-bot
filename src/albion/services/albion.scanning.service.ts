...
export class AlbionScanningService {
  ...
  async gatherCharacters(
    ...
    const statusMessage = await getChannel(message).send(
      `${emoji} Gathering ${length} characters from Albion API... (attempt #${tries})`,
    );

    for (const member of guildMembers) {
      characterPromises.push(
        this.albionApiService.getCharacterById(member.characterId),
      );
    }
  ...
  async roleInconsistencies(
    ...
    for (const member of guildMembers) {
      ...
      const inconsistencies =
        await this.checkRoleInconsistencies(discordMember);
