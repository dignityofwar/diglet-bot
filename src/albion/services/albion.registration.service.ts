...
export class AlbionRegistrationService implements OnApplicationBootstrap {
  ...
  async getInfo(
    ...
    return {
      ...
      character: await this.albionApiService.getCharacter(
        characterName,
        server,
      ),
      server,
  ...
  async validate(data: RegistrationData): Promise<void> {
    this.logger.debug(
      `Checking if registration attempt for "${data.character.Name}" is valid`,
    );
    ...
    this.logger.debug(
      `Registration attempt for "${data.character.Name}" is valid!`,
    );
  }
  ...
  private async checkIfInGuild(data: RegistrationData) {
    ...
    this
      .throwError(`Sorry <@${data.discordMember.id}>, the character **${data.character.Name}** has not been detected in the ${data.serverEmoji} **${data.guildName}** Guild.
...
- ⏳ **Please wait __~30 minutes__ and try again**.`);
  ...
  private async registerCharacter(
    data: RegistrationData,
    channel: TextChannel,
  ) {
    // Add roles based on guild membership
    const memberRole =
      data.server === AlbionServer.AMERICAS
    ...
    try {
      await data.discordMember.roles.add(
        await this.discordService.getRoleViaMember(
          data.discordMember,
          memberRole,
        ),
      );
    ...
    const rolesChannel =
      data.server === AlbionServer.AMERICAS
        ? this.config.get("discord.channels.albionUSRoles")
        : this.config.get("discord.channels.albionRoles");
    const announcementChannel =
      data.server === AlbionServer.AMERICAS
        ? this.config.get("discord.channels.albionUSAnnouncements")
        : this.config.get("discord.channels.albionAnnouncements");

    const pingRoles =
      data.server === AlbionServer.AMERICAS
        ? this.config.get("albion.pingLeaderRolesUS")
        : this.config.get("albion.pingLeaderRoles");
