...
describe("AlbionScanningService", () => {
  ...
  beforeEach(async () => {
    mockCharacterUS = TestBootstrapper.getMockAlbionCharacter(
      AlbionServer.AMERICAS,
    );
    mockCharacterEU = TestBootstrapper.getMockAlbionCharacter(
      AlbionServer.EUROPE,
    );
  ...
  describe("Scanning workflow", () => {
    ...
    it("should send number of members on record", async () => {
      ...
      service.gatherCharacters = jest
        .fn()
        .mockResolvedValueOnce([mockCharacterUS, mockCharacterUS]);
    ...
    it("should properly communicate scan progress, EU", async () => {
      ...
      service.gatherCharacters = jest
        .fn()
        .mockResolvedValueOnce([mockCharacterEU]);
      service.removeLeavers = jest.fn().mockResolvedValueOnce([]);
      service.reverseRoleScan = jest.fn().mockResolvedValueOnce([]);
      service.roleInconsistencies = jest.fn().mockResolvedValueOnce([]);
  ...
  describe("Leavers scanning", () => {
    it("should properly handle guild only leavers who have joined a new guild, US", async () => {
      ...
      expect(mockDiscordMessage.channel.send).toHaveBeenCalledWith(
        `- 🇺🇸 👋 <@${mockDiscordUser.id}>'s character **${mockCharacterUS.Name}** has left the Guild but remains on the Discord server. Their roles and registration status have been stripped.`,
      );
    ...
    it("should properly handle guild only leavers who have joined a new guild, EU", async () => {
      ...
      expect(mockDiscordMessage.channel.send).toHaveBeenCalledWith(
        `- 🇪🇺 👋 <@${mockDiscordUser.id}>'s character **${mockCharacterEU.Name}** has left the Guild but remains on the Discord server. Their roles and registration status have been stripped.`,
      );
    ...
    it("should properly handle server only leavers, US", async () => {
      ...
      expect(mockDiscordMessage.channel.send).toHaveBeenCalledWith(
        `- 🇺🇸 ‼️🫥️ Discord member for Character **${mockCharacterUS.Name}** has left the DIG Discord server. Their registration status has been removed. **They require booting from the Guild!**`,
      );
    ...
    it("should properly handle server only leavers, EU", async () => {
      ...
      expect(mockDiscordMessage.channel.send).toHaveBeenCalledWith(
        `- 🇪🇺 ‼️🫥️ Discord member for Character **${mockCharacterEU.Name}** has left the DIG Discord server. Their registration status has been removed. **They require booting from the Guild!**`,
      );
    ...
    it("should properly handle leavers for both server and guild, US", async () => {
      ...
      expect(mockDiscordMessage.channel.send).toHaveBeenCalledWith(
        `- 🇺🇸 💁 Character / Player ${mockCharacterUS.Name} has left **both** the DIG server and the Guild. They are dead to us now 💅`,
      );
    ...
    it("should properly handle leavers for both server and guild, EU", async () => {
      ...
      expect(mockDiscordMessage.channel.send).toHaveBeenCalledWith(
        `- 🇪🇺 💁 Character / Player ${mockCharacterEU.Name} has left **both** the DIG server and the Guild. They are dead to us now 💅`,
      );
