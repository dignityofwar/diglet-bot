...
describe('ThanosSnapCommand', () => {
  ...
  const mockInteraction = TestBootstrapper.getMockDiscordInteraction(
    '12345',
    mockDiscordUser,
  ...
  describe('onThanosSnapCommand', () => {
    ...
    it('should send a dry run message if dryRun is true', async () => {
      ...
      expect(mockInteraction[0].channel.send).toHaveBeenCalledWith(
        '## This is a dry run! No members will be kicked!',
      );
      expect(purgeService.startPurge).toHaveBeenCalled();
    });
