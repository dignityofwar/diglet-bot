...
describe('RoleMetricsService', () => {
  ...
  beforeEach(async () => {
    mockRoleMetricsRepository = TestBootstrapper.getMockRepositoryInjected(
    ...
    mockRoleMetricsRepository = module.get(
      getRepositoryToken(RoleMetricsEntity),
    );
...
- Onboarded: **112**
- Community Games
  - Albion Online: **90** (80.4%)
  - Some Game: **50** (44.6%)
  - Foxhole: **10** (8.9%)
- Rec Games
  - Rec/BestGameEver: **90** (80.4%)
  - Rec/Mediocre Game: **32** (28.6%)
  - Rec/WorstGameEver: **5** (4.5%)
`;
    ...
    beforeEach(() => {
      ...
      roleMetricsService.enumerateRoleIds = jest
        .fn()
        .mockResolvedValue(mockRoleList);
  ...
  describe('enumerateRoleIds', () => {
    ...
    it('should return the onboarded role', async () => {
      ...
      expect(roles.onboardedRole.id).toEqual(
        TestBootstrapper.mockOnboardedRoleId,
      );
    });
    ...
    it('should filter out ignored rec game roles', async () => {
      ...
      expect(
        roles.recGameRoles.get(TestBootstrapper.mockRecPS2LeaderId),
      ).toBeUndefined();
    });
    ...
    it('should return correct community game roles', async () => {
      ...
      expect(
        roles.communityGameRoles.get(TestBootstrapper.mockAlbionOnlineId),
      ).toBeDefined();

      // Expect that the role Id "234567890123456789" (Foxhole) is included in the roles
      expect(
        roles.communityGameRoles.get(TestBootstrapper.mockFoxholeId),
      ).toBeDefined();

      // Expect that the role Id "345678901234567890" (Rec/PS2/Leader) is not included in the roles
      expect(roles.communityGameRoles).not.toContain(
        TestBootstrapper.mockRecPS2LeaderId,
      );
    });
    ...
    it('should return the correct rec game roles', async () => {
      ...
      expect(
        roles.recGameRoles.get(TestBootstrapper.mockRecBestGameEverId),
      ).toBeDefined();

      // Expect the role ID "345678901234567891" (Albion Online) is NOT included in the roles
      expect(
        roles.recGameRoles.get(TestBootstrapper.mockAlbionOnlineId),
      ).toBeUndefined();
    });
  });
  ...
  describe('enumerateRoleMetrics', () => {
    beforeEach(() => {
      ...
      const hasMockRole = (id: string, role: string) => {
        return (
          activeMemberRoleMappings[id] && activeMemberRoleMappings[id][role]
        );
      };
    ...
    it('should properly record the role metrics', async () => {
      ...
      expect(
        mockRoleMetricsRepository.getEntityManager().persistAndFlush,
    ...
    it('should delete the previous role metrics', async () => {
      ...
      mockRoleMetricsRepository.find = jest
        .fn()
        .mockResolvedValue([mockRoleMetricsEntityToday]);
      await roleMetricsService.enumerateRoleMetrics(mockRoleList, mockGuild);

      expect(
        mockRoleMetricsRepository.getEntityManager().removeAndFlush,
      ).toHaveBeenCalledWith([mockRoleMetricsEntityToday]);
    });
  });
