...
describe("JoinerLeaverService", () => {
  ...
  beforeEach(async () => {
    mockJoinerLeaverRepository = TestBootstrapper.getMockRepositoryInjected(
      createMockJoinerLeaverEntity(),
    );
    mockJoinerLeaverStatisticsRepository =
      TestBootstrapper.getMockRepositoryInjected(
        mockJoinerLeaverStatisticsEntity,
      );
  ...
  describe("recordJoiner", () => {
    ...
    it("should record a new joiner", async () => {
      ...
      expect(mockJoinerLeaverRepository.findOne).toHaveBeenCalledWith({
        discordId: mockGuildMember.id,
      });

      expect(
        mockJoinerLeaverRepository.getEntityManager().persistAndFlush,
      ...
      expect(joinerLeaverService["logger"].log).toHaveBeenCalledWith(
        `Recorded joiner ${mockGuildMember.user.tag} (${mockGuildMember.id})`,
      );
    });

    it("should record a previous joiner", async () => {
      mockJoinerLeaverRepository.findOne = jest
        .fn()
        .mockResolvedValue(createMockJoinerLeaverEntity());
    ...
    it("should correctly increment a rejoiner", async () => {
      ...
      mockJoinerLeaverRepository.findOne = jest
        .fn()
        .mockResolvedValue(rejoinedLeaver);
  ...
  describe("recordLeaver", () => {
    ...
    it("should record a leaver", async () => {
      ...
      expect(joinerLeaverService["logger"].log).toHaveBeenCalledWith(
        `Recorded leaver ${mockGuildMember.user.tag} (${mockGuildMember.id})`,
      );
    });
  });
  ...
  it("should error if a leaver is not found", async () => {
    ...
    expect(joinerLeaverService["logger"].error).toHaveBeenCalledWith(
      `Attempted to record leaver ${mockGuildMember.user.tag} (${mockGuildMember.id}) but they were not found in the database.`,
    );
  });
...
- ⏳ Average Time to Leave: **1d 13h 37m**`;
    ...
    beforeEach(() => {
      mockJoinerLeaverStatisticsRepository.findOne = jest
        .fn()
        .mockResolvedValue(mockJoinerLeaverStatisticsEntity);
    });
    ...
    it("should handle error during enumeration", async () => {
      joinerLeaverService.enumerateJoinerLeavers = jest
        .fn()
      ...
      expect(
        mockJoinerLeaverStatisticsRepository.findOne,
      ).not.toHaveBeenCalled();
      expect(mockChannel.send).toHaveBeenCalledWith(
        "Error enumerating joiner leaver records. Error: Test error",
      );
    });
    ...
    it("should handle no statistics found", async () => {
      ...
      expect(mockJoinerLeaverStatisticsRepository.findOne).toHaveBeenCalled();
      expect(mockChannel.send).toHaveBeenCalledWith(
        "No joiner leaver statistics found!",
      );
    });
  });
  ...
  describe("enumerateJoinerLeavers", () => {
    ...
    beforeEach(() => {
      mockJoinerLeaveEntities = [
        ...
        {
          ...createMockJoinerLeaverEntity(),
          joinDate: generateDateInPast(20),
          leaveDate: generateDateInPast(1),
        },
        // 2x early leavers
        {
          ...createMockJoinerLeaverEntity(),
          joinDate: generateDateInPast(4),
          leaveDate: generateDateInPast(3.9),
        },
        {
          ...createMockJoinerLeaverEntity(),
          joinDate: generateDateInPast(4),
          leaveDate: generateDateInPast(3.4),
        },
        // Rejoiner
        {
          ...createMockJoinerLeaverEntity(),
      ...
      mockJoinerLeaverRepository.findAll = jest
        .fn()
        .mockResolvedValue(mockJoinerLeaveEntities);
    });
    ...
    it("should enumerate joiner leavers", async () => {
      ...
      expect(
        mockJoinerLeaverStatisticsRepository.getEntityManager().persistAndFlush,
      ).toHaveBeenCalledWith({
        joiners: 6,
        leavers: 3,
        rejoiners: 1,
    ...
    describe("calculateAverageTimeToLeave", () => {
      it("should calculate the average time to leave (5d)", () => {
        ...
        const averageTime = joinerLeaverService.calculateAvgTimeToLeave(
          simpleJoinerLeaveEntities,
        );
